/**
 * Zyrix App — InnerHeader Component
 * هيدر موحد لجميع الصفحات الداخلية
 * يحتوي على: زر رجوع + عنوان الصفحة + زر القائمة الجانبية
 */
...
function BackIcon({ color = COLORS.white }: { color?: string }) {
  return (
...
// ─── Side Menu (نفس القائمة الموجودة في dashboard) ──
...
interface InnerHeaderProps {
  title: string;
  accentColor?: string; // لون مميز اختياري لكل صفحة
}

export function InnerHeader({ title, accentColor }: InnerHeaderProps) {
  const router  = useRouter();
      ...
      <View style={[
        ...
        <View style={h.row}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <BackIcon color={accentColor ?? COLORS.white} />
          </TouchableOpacity>
