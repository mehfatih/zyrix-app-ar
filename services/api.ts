...
export const analyticsApi = {
  getData: (range: '7d' | '30d' | '90d' = '30d') => {
    ...
    return request<{ range: string; kpi: { volume: number; successRate: number; avgTx: number; customers: number }; volume: { label: string; value: number }[]; successRate: { label: string; value: number }[]; methods: { label: string; value: number }[]; countries: { label: string; value: number }[] }>(`/api/analytics?range=${range}`);
  },
};
...
export const paymentLinksApi = {
  list: (status?: string) => {
    ...
    const qs = status ? `?status=${status}` : '';
    return request<{ links: Array<{ id: string; linkId: string; amount: string; currency: string; title: string; description: string | null; status: string; expiresAt: string | null; paidAt: string | null; createdAt: string; paymentUrl?: string }> }>(`/api/payment-links${qs}`);
  },
  create: (data: { amount: number; title: string; currency?: string; description?: string; expiresInHours?: number }) =>
    request<{ id: string; linkId: string; amount: string; currency: string; title: string; status: string; paymentUrl: string; createdAt: string }>('/api/payment-links', { method: 'POST', body: JSON.stringify(data) }),
...
export const subscriptionsApi = {
  list: (status?: string) => {
    ...
    const qs = status ? `?status=${status}` : '';
    return request<{ subscriptions: Array<{ id: string; subscriptionId: string; customerName: string; amount: string; currency: string; interval: string; title: string; status: string; nextBillingDate: string; billingCount: number; createdAt: string }> }>(`/api/subscriptions${qs}`);
  },
  create: (data: { customerName: string; amount: number; interval: string; title: string; customerEmail?: string; customerPhone?: string }) =>
    request<{ subscriptionId: string; status: string }>('/api/subscriptions', { method: 'POST', body: JSON.stringify(data) }),
  pause: (subId: string) => request<{ status: string }>(`/api/subscriptions/${subId}/pause`, { method: 'PUT' }),
  resume: (subId: string) => request<{ status: string }>(`/api/subscriptions/${subId}/resume`, { method: 'PUT' }),
  cancel: (subId: string) => request<{ status: string }>(`/api/subscriptions/${subId}/cancel`, { method: 'PUT' }),
};
...
export const revenueGoalsApi = {
  list: () => request<{ goals: Array<{ id: string; targetAmount: string; currentAmount: number; currency: string; period: string; status: string; progress: number; daysLeft: number; startDate: string; endDate: string }> }>('/api/revenue-goals'),
  ...
  delete: (id: string) => request<{ success: boolean }>(`/api/revenue-goals/${id}`, { method: 'DELETE' }),
};
...
export const expensesApi = {
  list: (params?: { category?: string; from?: string; to?: string }) => {
    ...
    const qs = q.toString();
    return request<{ expenses: Array<{ id: string; amount: string; currency: string; category: string; title: string; description: string | null; date: string; recurring: boolean }>; summary: { totalExpenses: number; totalRevenue: number; netProfit: number; byCategory: Array<{ category: string; total: number; count: number }> } }>(`/api/expenses${qs ? `?${qs}` : ''}`);
  },
  ...
  delete: (id: string) => request<{ success: boolean }>(`/api/expenses/${id}`, { method: 'DELETE' }),
};

export const invoicesApi = {
  list: (status?: string) => {
    const qs = status ? `?status=${status}` : '';
    return request<{ invoices: Array<{ id: string; invoiceId: string; customerName: string; items: string; subtotal: string; tax: string; total: string; currency: string; status: string; dueDate: string | null; createdAt: string }> }>(`/api/invoices${qs}`);
  },
  getById: (id: string) => request<{ id: string; invoiceId: string; customerName: string; items: string; subtotal: string; tax: string; total: string; status: string; merchant: { name: string; company: string; iban: string | null } }>(`/api/invoices/${id}`),
  ...
  send: (invoiceId: string) => request<{ status: string }>(`/api/invoices/${invoiceId}/send`, { method: 'PUT' }),
  markPaid: (invoiceId: string) => request<{ status: string }>(`/api/invoices/${invoiceId}/mark-paid`, { method: 'PUT' }),
};

export const transfersApi = {
  list: () => request<{ sent: Array<{ id: string; transferId: string; amount: string; currency: string; description: string | null; status: string; createdAt: string; toMerchant: { merchantId: string; name: string } }>; received: Array<{ id: string; transferId: string; amount: string; currency: string; description: string | null; status: string; createdAt: string; fromMerchant: { merchantId: string; name: string } }> }>('/api/transfers'),
...
export const apiKeysApi = {
  list: () => request<{ data: Array<{ id: string; name: string; keyPrefix: string; isActive: boolean; lastUsedAt: string | null; createdAt: string }> }>('/api/api-keys'),
  create: (name?: string) =>
    request<{ data: { id: string; name: string; keyPrefix: string; isActive: boolean; createdAt: string; fullKey: string }; success: boolean }>('/api/api-keys', { method: 'POST', body: JSON.stringify({ name }) }),
  revoke: (id: string) => request<{ success: boolean }>(`/api/api-keys/${id}`, { method: 'DELETE' }),
};

export const webhooksApi = {
  list: () => request<{ success: boolean; data: Array<{ id: string; name: string; url: string; events: string[]; isActive: boolean; lastTriggeredAt: string | null; failureCount: number; createdAt: string }> }>('/api/webhooks'),
  create: (data: { url: string; events: string[]; name?: string }) =>
    request<{ success: boolean; data: { id: string; name: string; url: string; events: string[]; isActive: boolean; createdAt: string } }>('/api/webhooks', { method: 'POST', body: JSON.stringify(data) }),
  delete: (id: string) => request<{ success: boolean }>(`/api/webhooks/${id}`, { method: 'DELETE' }),
  test: (id: string) => request<{ success: boolean; data: { message: string; status: string } }>(`/api/webhooks/${id}/test`, { method: 'POST' }),
};

export const codApi = {
  list: (status?: string) => {
    const qs = status ? `?status=${status}` : '';
    return request<{ success: boolean; data: Array<{ id: string; orderId: string; customerName: string; amount: number; currency: string; address: string; phone: string | null; description: string | null; status: string; collectedAmount: number; collectedAt: string | null; notes: string | null; createdAt: string }> }>(`/api/cod${qs}`);
  },
  create: (data: { customerName: string; amount: number; currency?: string; address: string; phone?: string; description?: string }) =>
    request<{ success: boolean; data: { id: string; orderId: string; status: string } }>('/api/cod', { method: 'POST', body: JSON.stringify(data) }),
  markCollected: (id: string, data?: { collectedAmount?: number; notes?: string }) =>
    request<{ success: boolean; data: { id: string; status: string; collectedAmount: number } }>(`/api/cod/${id}/collected`, { method: 'PUT', body: JSON.stringify(data || {}) }),
  summary: () =>
