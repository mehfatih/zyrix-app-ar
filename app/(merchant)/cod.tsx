  ...
  return (
    <View style={[cd.container, { backgroundColor: colors.bg, borderColor: colors.border }]}>
      ...
      {order.status === 'PENDING' || order.status === 'OUT_FOR_DELIVERY' ? (
        <TouchableOpacity
          style={cd.collectBtn}
          onPress={() => onCollect(order.id, order.amount)}
        >
...
export default function CODScreen() {
  ...
  const handleCreate = async () => {
    if (!form.customerName.trim()) { Alert.alert('خطأ', 'أدخل اسم العميل'); return }
    if (!form.amount || isNaN(parseFloat(form.amount))) { Alert.alert('خطأ', 'أدخل مبلغاً صحيحاً'); return }
    if (!form.address.trim()) { Alert.alert('خطأ', 'أدخل العنوان'); return }
    ...
    try {
      ...
      Alert.alert('✓', 'تم إنشاء طلب الكاش عند الاستلام')
      setShowCreate(false)
  ...
  const handleCollect = async () => {
    ...
    setCollecting(true)
    try {
      await codApi.markCollected(showCollect.id, {
        collectedAmount: showCollect.amount,
        notes: collectNotes.trim() || undefined,
      })
      Alert.alert('✓', 'تم تأكيد استلام النقود')
      setShowCollect(null)
      setCollectNotes('')
      fetchData()
    } catch (err: any) {
  ...
  const renderItem = ({ item, index }: ListRenderItemInfo<CODOrder>) => (
    <CODCard order={item} index={index} onCollect={(id, amount) => setShowCollect({ id, amount })} />
  )
    ...
    <>
      <View style={st.pageHeader}>
        <View style={[st.headerRow, isRTL && st.headerRowRTL]}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[st.pageTitle, isRTL && st.textRight]}>كاش عند الاستلام</Text>
            <Text style={[st.pageSubtitle, isRTL && st.textRight]}>COD — Cash on Delivery</Text>
          </View>
      ...
      <View style={[st.kpiRow, isRTL && st.kpiRowRTL]}>
        <View style={[st.kpiCard, { backgroundColor: 'rgba(5,150,105,0.15)', borderColor: 'rgba(5,150,105,0.3)' }]}>
          <Text style={st.kpiLabel}>إجمالي المحصّل</Text>
          <Text style={[st.kpiValue, { color: COLORS.success }]}>{summary.totalCollected.toLocaleString('en-US', { minimumFractionDigits: 0 })} ر.س</Text>
        </View>
        <View style={[st.kpiCard, { backgroundColor: 'rgba(245,158,11,0.15)', borderColor: 'rgba(245,158,11,0.3)' }]}>
          <Text style={st.kpiLabel}>قيد الانتظار</Text>
          <Text style={[st.kpiValue, { color: COLORS.warning }]}>{summary.totalPending.toLocaleString('en-US', { minimumFractionDigits: 0 })} ر.س</Text>
        </View>
