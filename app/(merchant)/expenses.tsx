...
const CAT_ICONS: Record<string, string> = { rent: '🏢', salary: '💼', supplies: '📦', marketing: '📢', utilities: '⚡', other: '📋' }
...
const DEMO_SUMMARY = {
  ...
  byCategory: [
    ...
    { category: 'rent', total: 2200, count: 1 },
    { category: 'marketing', total: 850, count: 1 },
    { category: 'utilities', total: 320, count: 1 },
    { category: 'supplies', total: 180, count: 1 },
    { category: 'other', total: 1200, count: 1 },
  ]
}
...
const CARD_BG = [
  ...
  { bg: 'rgba(236, 72, 153, 0.1)', border: 'rgba(236, 72, 153, 0.25)' },
]
...
export default function ExpensesScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (res?.expenses?.length > 0) { setData(res) } else { setData({ expenses: DEMO_EXPENSES, summary: DEMO_SUMMARY }) }
    } catch (_e) { setData({ expenses: DEMO_EXPENSES, summary: DEMO_SUMMARY }) }
    ...
    <SafeAreaView style={st.safe}>
      <ScrollView contentContainerStyle={st.scroll} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchData() }} tintColor={COLORS.primary} />}>
        ...
        <View style={st.header}>
          <Text style={st.headerTitle}>{t('expenses.title')}</Text>
        ...
        <View style={st.kpiWrapper}>
          <View style={[st.kpiRow, isRTL && st.kpiRowRTL]}>
            ...
            <View style={[st.kpiCard, { backgroundColor: 'rgba(26, 86, 219, 0.15)', borderColor: 'rgba(26, 86, 219, 0.3)' }]}>
              ...
              <Text style={[st.kpiValue, { color: summary.netProfit >= 0 ? COLORS.primaryLight : COLORS.danger }]}>{summary.netProfit.toLocaleString('en-US')} ر.س</Text>
            </View>
          </View>
        </View>
        ...
        <View style={st.chartContainer}>
          <View style={st.chartBarGroup}>
            <View style={st.chartBarTrack}><View style={[st.chartBarFill, { backgroundColor: COLORS.success, height: `${Math.max((summary.totalRevenue / maxKpi) * 100, 8)}%` }]} /></View>
            <Text style={[st.chartBarLabel, { color: COLORS.success }]}>{t('expenses.total_revenue')}</Text>
          </View>
          <View style={st.chartBarGroup}>
            <View style={st.chartBarTrack}><View style={[st.chartBarFill, { backgroundColor: COLORS.danger, height: `${Math.max((summary.totalExpenses / maxKpi) * 100, 8)}%` }]} /></View>
            <Text style={[st.chartBarLabel, { color: COLORS.danger }]}>{t('expenses.total_expenses')}</Text>
          </View>
          <View style={st.chartBarGroup}>
            <View style={st.chartBarTrack}><View style={[st.chartBarFill, { backgroundColor: COLORS.primaryLight, height: `${Math.max((Math.abs(summary.netProfit) / maxKpi) * 100, 8)}%` }]} /></View>
            <Text style={[st.chartBarLabel, { color: COLORS.primaryLight }]}>{t('expenses.net_profit')}</Text>
          </View>
        </View>
          ...
          return (
            <View key={exp.id} style={[st.card, { backgroundColor: colors.bg, borderColor: colors.border }]}>
              <View style={[st.cardRow, isRTL && st.cardRowRTL]}>
                <View style={st.cardIcon}><Text style={{ fontSize: 18 }}>{CAT_ICONS[exp.category] || '📋'}</Text></View>
                <View style={{ flex: 1 }}>
                  <Text style={st.cardTitle}>{exp.title}</Text>
                  <Text style={st.cardCat}>{t(`expenses.${exp.category}`) || exp.category} · {new Date(exp.date).toLocaleDateString('ar-SA')}</Text>
                </View>
      ...
      <Modal visible={showCreate} transparent animationType="slide">
        <View style={st.modalOverlay}><View style={st.modal}>
          ...
          <TextInput placeholder={t('expenses.title')} value={form.title} onChangeText={v => setForm({...form, title: v})} style={st.input} placeholderTextColor={COLORS.textMuted} textAlign="right" />
          <TextInput placeholder="المبلغ" value={form.amount} onChangeText={v => setForm({...form, amount: v})} style={st.input} placeholderTextColor={COLORS.textMuted} keyboardType="decimal-pad" textAlign="right" />
          <View style={st.catGrid}>
            {CATEGORIES.map(c => (
              <TouchableOpacity key={c} style={[st.catBtn, form.category === c && st.catBtnActive]} onPress={() => setForm({...form, category: c})}>
                <Text style={[st.catBtnText, form.category === c && st.catBtnTextActive]}>{CAT_ICONS[c]} {t(`expenses.${c}`)}</Text>
              </TouchableOpacity>
            ))}
          </View>
          <View style={st.modalActions}>
            ...
            <TouchableOpacity style={st.modalSubmit} onPress={handleCreate} disabled={creating}><Text style={{ color: COLORS.white, fontWeight: '700' }}>{creating ? '...' : t('expenses.add')}</Text></TouchableOpacity>
          </View>
        </View></View>
      </Modal>
    </SafeAreaView>
...
const st = StyleSheet.create({
  ...
  header: { flexDirection: isRTL ? 'row' : 'row-reverse', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingTop: 12, paddingBottom: 10, backgroundColor: 'rgba(13, 148, 136, 0.12)', borderBottomWidth: 1, borderBottomColor: 'rgba(13, 148, 136, 0.3)' },
