...
import {
  ...
  ActivityIndicator, I18nManager, SafeAreaView,
} from 'react-native';
...
const FIELD_COLORS = [
  { border: 'rgba(26, 86, 219, 0.40)',  bg: 'rgba(26, 86, 219, 0.07)',  accent: '#60A5FA' },
  { border: 'rgba(13, 148, 136, 0.40)', bg: 'rgba(13, 148, 136, 0.07)', accent: '#2DD4BF' },
  { border: 'rgba(139, 92, 246, 0.40)', bg: 'rgba(139, 92, 246, 0.07)', accent: '#A78BFA' },
];
...
function PasswordField({ label, value, onChangeText, placeholder, isRTL, error, colorIndex }: {
  ...
  return (
    <View style={styles.fieldContainer}>
      ...
      <View style={[
        styles.inputRow, isRTL && styles.inputRowRTL,
        { backgroundColor: fc.bg, borderColor: error ? COLORS.danger : fc.border },
      ]}>
        <TextInput
          style={[styles.input, isRTL && styles.inputRTL]}
          value={value} onChangeText={onChangeText}
          placeholder={placeholder} placeholderTextColor={COLORS.inputPlaceholder}
        ...
        <TouchableOpacity onPress={() => setVisible(!visible)} style={styles.eyeButton} activeOpacity={0.7}>
          {visible ? <EyeIcon size={18} color={fc.accent} /> : <EyeOffIcon size={18} color={COLORS.textMuted} />}
        </TouchableOpacity>
      </View>
...
export default function ChangePasswordScreen() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        t('change_password.success_title'), t('change_password.success_message'),
  ...
  return (
    <SafeAreaView style={styles.container}>
      <InnerHeader title={t('change_password.title')} />
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.scrollContent}
          ...
          <PasswordField
            label={t('change_password.current_password')} value={currentPassword}
          ...
          />
          <PasswordField
            label={t('change_password.new_password')} value={newPassword}
          ...
          />
          <PasswordField
            label={t('change_password.confirm_password')} value={confirmPassword}
...
const styles = StyleSheet.create({
  ...
  fieldLabel:           { fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.semibold, marginBottom: SPACING.sm },
  textRTL:              { textAlign: 'right' },
  inputRow:             { flexDirection: 'row', alignItems: 'center', borderRadius: RADIUS.md, borderWidth: 1.5, paddingHorizontal: SPACING.md, height: 52 },
  inputRowRTL:          { flexDirection: 'row-reverse' },
  input:                { flex: 1, fontSize: FONT_SIZE.base, color: COLORS.textPrimary, paddingVertical: 0, textAlign: I18nManager.isRTL ? 'right' : 'left' },
  ...
  requirementsTitle:    { fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.semibold, color: COLORS.textSecondary, marginBottom: SPACING.md },
  requirementsGrid:     { gap: SPACING.sm },
  reqItem:              { flexDirection: 'row', alignItems: 'center', paddingHorizontal: SPACING.md, paddingVertical: SPACING.sm, borderRadius: RADIUS.md, gap: SPACING.sm },
  reqDot:               { width: 8, height: 8, borderRadius: 4 },
  reqText:              { fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.medium },
  submitButton:         { backgroundColor: COLORS.primary, borderRadius: RADIUS.md, height: 52, alignItems: 'center', justifyContent: 'center' },
