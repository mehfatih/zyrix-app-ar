...
export default function TransactionsScreen() {
  ...
  const renderItem = ({ item }: ListRenderItemInfo<Transaction>) => (
    <TransactionRow {...item} onPress={() => handleRowPress(item.id)} />
  )
    ...
    <>
      <View style={styles.header}>
        <View style={[styles.headerTop, isRTL && styles.headerTopRTL]}>
          ...
          <TouchableOpacity
            style={styles.csvBtn}
            onPress={() => {
              Linking.openURL('https://api.zyrix.co/api/export/transactions?format=csv').catch(() => {})
            }}
          >
      ...
      <View style={[styles.kpiRow, isRTL && styles.kpiRowRTL]}>
        ...
        <KpiCard
          ...
          icon="📋" color={COLORS.chart.purple}
        ...
        <View style={styles.filterRow}>
          {FILTERS.map((f) => {
            ...
            return (
              <TouchableOpacity
                key={f.key}
                style={[styles.filterTab, isActive && { backgroundColor: f.activeBg, borderColor: f.activeBorder }]}
                onPress={() => setFilter(f.key)}
              >
                <Text style={[styles.filterTabText, isActive && { color: f.color, fontWeight: '700' }]}>
                  {t(f.labelKey)}
                </Text>
              </TouchableOpacity>
            )
...
const styles = StyleSheet.create({
  ...
  listContent:    { },
  header:         { paddingHorizontal: 16, paddingTop: 16, paddingBottom: 14, backgroundColor: 'rgba(13,148,136,0.12)', borderBottomWidth: 1, borderBottomColor: 'rgba(13,148,136,0.3)' },
  headerTop:      { flexDirection: 'row', alignItems: 'center', gap: 10 },
  headerTopRTL:   { flexDirection: 'row-reverse' },
  subtitle:       { fontSize: 13, color: COLORS.textPrimary, fontWeight: '600' },
  ...
  chartBarTrack:  { width: 28, height: 70, backgroundColor: COLORS.surfaceBg, borderRadius: 6, justifyContent: 'flex-end', overflow: 'hidden' },
  chartBarFill:   { width: '100%', borderRadius: 6 },
  chartBarLabel:  { fontSize: 9, fontWeight: '700', textAlign: 'center' },
  searchWrapper:  { paddingHorizontal: 12, paddingVertical: 10, backgroundColor: COLORS.darkBg },
  searchInput:    { height: 44, backgroundColor: COLORS.cardBg, borderRadius: 12, paddingHorizontal: 16, fontSize: 14, color: COLORS.textPrimary, borderWidth: 1, borderColor: COLORS.border },
  filterWrapper:  { backgroundColor: COLORS.surfaceBg, paddingHorizontal: 12, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: COLORS.border },
  filterRow:      { flexDirection: isRTL ? 'row' : 'row-reverse', gap: 8 },
